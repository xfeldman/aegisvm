...
Installs both `aegisvm` and `aegisvm-agent-kit` packages, plus dependencies (`virtiofsd`, `e2fsprogs`, `iptables`, `iproute2`). Binaries go to `/usr/bin/` and `/usr/lib/aegisvm/`, kernel to `/usr/share/aegisvm/kernel/`.
